 ...
 * Komponen utama
 */
function UserCreateMenuAccess({ isSkeletonShow, userId, onBackStep }) {
  ...
  /**
   * Mengecek props userId bernilai null atau tidak
   * jika null kembalikan ke step sebelumnya
   */
  React.useEffect(() => {
    if (userId === null) onBackStep();
  }, [userId, onBackStep]);
  ...
  const handleSubmit = e => {
    ...
    dispatch(apiCreateUserMenuAccess(menuItemValues, menuSubItemValues))
      .then(() => {
    ...
    <div className={classes.root}>
      <form>
        <Grid container spacing={3}>
          ...
          {Boolean(menuItems.data === null || menuSubItems.data === null) ? (
            <Grid item xs={12}>
              <Box
                mt={3}
                display="flex"
...
export default UserCreateMenuAccess;
